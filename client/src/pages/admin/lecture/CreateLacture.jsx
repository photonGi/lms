...
const CreateLacture = () => {
    ...
    const [createLacture, { data, isLoading, isSuccess, error }] =
        useCreateLectureMutation();

    const { data: lectureData, isLoading: lectureLoading, isError: lectureError,
        refetch,
     } = useGetCourseLectureQuery(courseId);

    const createLectureHandler = async () => {
        await createLacture({ lectureTitle, courseId })
    }
    useEffect(() => {
        if (isSuccess) {
            refetch();
