    ...
    <div className='max-w-7xl mx-auto p-4 mt-20'>
      ...
      <div className='flex justify-between mb-4'>
        ...
        >
          {completed ? <div className='flex items-center'>
            <CheckCircle className='h-4 w-4 mr-2' /><span>Completed</span> </div> : "Mark as completed"}
        </Button>
      </div>
      ...
      <div className='flex flex-col md:flex-row gap-6'>
        ...
        <div className='flex-1 md:w-3/5 h-fit rounded-lg shadow-lg p-4'>
          ...
          {/* Display current watching lecture titile */}
          <div className='mt-2'>
            <h3 className='font-medium text-lg'>
              {
                `Lecture ${courseDetails.lectures.findIndex((lec) => lec._id ===
                  (currentLecture?._id || initialLecture._id)) + 1} 
          ...
          <div className='flex-1 overflow-y-auto'>
            {
              courseDetails?.lectures.map((lecture) => {
                <Card
                  key={lecture_.id}
                  className={`mb-3 hover:cursor-pointer transition transform ${lecture._id === currentLecture._id ? 'bg-gray-200' : 'dark:bg-gary-600'}`}
                ...
                >
                  <CardContent className="flex items-center justify-between p-4">
                    <div className='flex items-center'>
                      ...
                      <div>
                        <CardTitle className="text-lg font-medidum">
                    ...
                    </div>
                    {
                      islectureCompleted(lecture._id) && (
                        <Badge variant={"outline"}
                          className='bg-green-200 text-green-600'>Completed</Badge>
                      )
                    }
                  </CardContent>
                </Card>
